...
public class MediumBattleSystem : MonoBehaviour
{
    ...
    public MediumBattleState state;
    ...
    void Update()
    {
        if (state == MediumBattleState.PLAYERTURN && !isPlayerAttacking)
        {
    ...
    IEnumerator SetupBattle()
    {
        ...
        state = MediumBattleState.PLAYERTURN;
        StartPlayerTurn();
    }
    ...
    void EndPlayerTurn()
    {
        ...
        state = MediumBattleState.ENEMYTURN;
    ...
    {
        if (state == MediumBattleState.ENEMYTURN)
        {
    ...
    void EndBattle()
    {
        if (state == MediumBattleState.WON)
        {
            if (enemiesDefeated >= maxEnemiesToDefeat)
            {
                state = MediumBattleState.GAMEOVER;
        ...
        else if (state == MediumBattleState.LOST)
        {
            state = MediumBattleState.GAMEOVER;
    ...
    {
        if (state != MediumBattleState.PLAYERTURN)
            return;
    ...
    public void ShowPauseScreen()
    {
        ...
        state = MediumBattleState.PAUSED;
    }
    ...
    {
        if (state == MediumBattleState.PAUSED)
        {
            Time.timeScale = 1f;
            HidePauseScreen();
