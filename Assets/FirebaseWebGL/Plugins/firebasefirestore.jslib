mergeInto(LibraryManager.library, {
    ...
    SignOut: function(objectName, callback, fallback) {
        ...
        firebase.auth().signOut()
            .then(() => {
                unityInstance.SendMessage(parsedObjectName, parsedCallback, "Sign-out successful");
            }).catch((error) => {
                unityInstance.SendMessage(parsedObjectName, parsedFallback, error.message);
            });
    },

    GetCurrentUser: function(objectName, callback, fallback) {
        var parsedObjectName = UTF8ToString(objectName);
        var parsedCallback = UTF8ToString(callback);
        var parsedFallback = UTF8ToString(fallback);
        ...
        if (user) {
            unityInstance.SendMessage(parsedObjectName, parsedCallback, JSON.stringify(user));
        } else {
            unityInstance.SendMessage(parsedObjectName, parsedFallback, "No user is signed in");
        }
    },

    SetUserData: function(userId, data, objectName, callback, fallback) {
        var parsedUserId = UTF8ToString(userId);
        var parsedData = JSON.parse(UTF8ToString(data));
        var parsedObjectName = UTF8ToString(objectName);
        var parsedCallback = UTF8ToString(callback);
        var parsedFallback = UTF8ToString(fallback);

        firebase.firestore().collection('users').doc(parsedUserId).set(parsedData)
            .then(() => {
                unityInstance.SendMessage(parsedObjectName, parsedCallback, "Document successfully written!");
            })
            .catch((error) => {
                unityInstance.SendMessage(parsedObjectName, parsedFallback, error.message);
            });
    ...
    GetUserData: function(userId, objectName, callback, fallback) {
        var parsedUserId = UTF8ToString(userId);
        var parsedObjectName = UTF8ToString(objectName);
        var parsedCallback = UTF8ToString(callback);
        var parsedFallback = UTF8ToString(fallback);

        firebase.firestore().collection('users').doc(parsedUserId).get()
            .then((doc) => {
                if (doc.exists) {
                    unityInstance.SendMessage(parsedObjectName, parsedCallback, JSON.stringify(doc.data()));
                } else {
                    unityInstance.SendMessage(parsedObjectName, parsedFallback, "No such document!");
                }
            })
            .catch((error) => {
                unityInstance.SendMessage(parsedObjectName, parsedFallback, error.message);
