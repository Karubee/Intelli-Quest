...
namespace FirebaseWebGL.Examples.Storage
{
    public class StorageExampleHandler : MonoBehaviour
    {
        ...
        public void UploadFileToStorage() => FirebaseStorage.UploadFileToStorage(filePathInputField.text,
            Convert.ToBase64String(Encoding.ASCII.GetBytes(fileDataInputField.text)), gameObject.name, "DisplayInfo", "DisplayErrorObject");

        public void DownloadFileFromStorage() => FirebaseStorage.DownloadFileFromStorage(filePathInputField.text, gameObject.name, "DisplayFile", "DisplayErrorObject");
